...
@Component
public class UpdateSpecialityCaptchaValidator implements Validator {

	Logger logger = LoggerFactory.getLogger(UpdateSpecialityCaptchaValidator.class);
	...
	public void validate(Object o, Errors errors) {
		 ...
		 * 
		 * if (file.isEmpty()) { errors.rejectValue("image",
		 * "NotEmpty.createSpecialityCaptchaForm.image"); }
		 */

		/*
		 * if (!StringUtils.isEmpty(specialityCaptcha.getCaptchaText()) &&
		 * specialityCaptcha.getSpeciality() > 0) { final SpecialityCaptchaDTO
		 * specialityCaptchaDTO =
		 * specialityCaptchaService.findByCaptchaTxtAndSpecialityId(
		 * specialityCaptcha.getCaptchaText(), specialityCaptcha.getSpeciality()); if
		 * (specialityCaptchaDTO == null) { errors.rejectValue("captchaText",
		 * "DataNotFound.createSpecialityCaptchaForm.captchaText"); }
		 * 
		 * }
		 */
	}
